...
class Mode4():
    ...
    def check_if_day_in_correct_format(self, day: str):
        while True:
            try:
    ...
    def get_start_day_input(self):
        ...
        start_day = self.check_if_day_in_correct_format(start_day)
    ...
    def get_end_day_input(self):
        ...
        if end_day != "brak":
            end_day = self.check_if_day_in_correct_format(end_day)
            end_day = end_day.replace("/", ".")

        return end_day


    def check_if_hours_in_correct_format(self, hours: str):
        while True:
            try:
    ...
    def get_hours_input(self):
        ...
        hours = self.check_if_hours_in_correct_format(hours)
    ...
    def count_weeks_between_dates(self, start_day: str, end_day: str):
    ...
    def create_lists_of_dates(self, start_day, end_day, start_hour, end_hour):
        start_dates_list = []
        end_dates_list = []
        ...
        if end_day != "brak":
            week_count = self.count_weeks_between_dates(start_day, end_day)
    ...
    def check_if_events_overlap(self, start_day, end_day, start_hour, end_hour):
    ...
    def confirm_the_change(self, inputs: dict):
        print(
            f"\nTwoje wydarzenie:\
            \nTyp: {inputs['Typ']}\
            \nTytuł: {inputs['Tytuł']}\
    ...
    def user_chose_option_4(self):
        inputs = self.make_inputs_into_dict()
        (input_dict, overlap) = inputs

        if len(overlap) == 0:
            self.add_new_event(input_dict)
        else:
            print(
