...
def check_if_day_in_correct_format(day: str):
    while True:
        try:
...
def add_new_event(input_dict: dict, file_path: str):
    choice = confirm_the_change(input_dict)

    if choice == "y":
        column_names = list(input_dict.keys())
        with open(
            f"{file_path}", "a", encoding="utf8", newline=""
        ) as csvfile:
...
def user_chose_option_4(file_path: str):
    ...
    if len(overlap) == 0:
        add_new_event(input_dict, file_path)
    else:
        print(
