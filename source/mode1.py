...
class Mode1():
    def check_if_user_input_is_correct_format(self, user_input: str):
        while True:
            try:
                if user_input != "teraz":
    ...
    def option_1_user_input(self):
        ...
        user_date = self.check_if_user_input_is_correct_format(user_input)

        return user_date


    def check_if_during_datetime_there_is_event(self, datetime_date: datetime):
        busy = 0

        for i in range(len(start_days)):
            for j in range(len(full_start[i])):
                if (datetime_date <= full_end[i][j]) and (
                    datetime_date >= full_start[i][j]
                ):
                    busy = 1
    ...
    def search_through_rest_of_day(self, datetime_date: datetime):
        possible_next_class = []
        for i in range(1, 21):
    ...
    def if_next_class_found(self, busy: bool, next_class, datetime_date: datetime):
        if busy == True:
    ...
    def find_next_class_if_no_class_when_specified(self, datetime_date: datetime):
        busy = False  # inicjalizacja zmiennej, która określa czy osoba będzie miała zajęcia po danej godzinie w określonym dniu
        next_class = ""

        possible_next_class = self.search_through_rest_of_day(datetime_date)

        for date in possible_next_class:
            for i in range(len(start_days)):
                for j in range(len(full_start[i])):
                    if (date <= full_end[i][j]) and (date >= full_start[i][j]):
        ...
        return self.if_next_class_found(busy, next_class, datetime_date)


    def instructions_whether_class_was_found(self, list_to_check: list, datetime_date: datetime):
        if list_to_check:
            # sprecyzowanie formy zajęć, jeśli dany przedmiot ma tylko jedną formę, str class_type_name jest pusty
            class_type_set = set()
            list_to_set_specified(
                class_type_list, class_type_set, subject_list, list_to_check[1]
            )
            ...
            result = self.find_next_class_if_no_class_when_specified(datetime_date)

            print(f"\nNie masz wtedy zajęć.\n{result}\n\n")


    def user_chose_option_1(self):
        user_given_date = self.option_1_user_input()
        ...
        event_list = self.check_if_during_datetime_there_is_event(user_given_date)

        self.instructions_whether_class_was_found(event_list, user_given_date)
