...
class Mode2():
    def generate_subject_options(self):
    ...
    def check_if_user_input_is_in_dict_keys(self, user_input: str, options: dict):
    ...
    def get_user_input_subject(self):
        possible_subjects = self.generate_subject_options()
        ...
        subject_input = self.check_if_user_input_is_in_dict_keys(user_input, possible_subjects)

        return subject_input


    def search_for_subject_corresponding_to_user_choice(self):
        possible_subjects = self.generate_subject_options()
        subject = self.get_user_input_subject()
    ...
    def search_for_number_of_different_class_types(self, class_type_set: set):
    ...
    def create_class_type_dict_for_classes_with_more_than_one_type(
        self, class_type_set: set, class_type_dict: dict
    ):
        number_type = 0
    ...
    def get_user_input_class_type(self, class_type_dict: dict):
        ...
        class_type_input = self.check_if_user_input_is_in_dict_keys(user_input, class_type_dict)
    ...
    def get_class_type_and_subject_name(self):
        class_type_set = set()
        class_type_dict = {}
        subject = self.search_for_subject_corresponding_to_user_choice()

        list_to_set_specified(class_type_list, class_type_set, subject_list, subject)
        type_choice = self.search_for_number_of_different_class_types(class_type_set)

        if type_choice == 1:
            self.create_class_type_dict_for_classes_with_more_than_one_type(
                class_type_set, class_type_dict
            )
            class_type = self.search_for_class_type_corresponding_to_user_choice(class_type_dict)
    ...
    def check_if_closest_date_exists(self, dates: list):
        if len(dates) != 0:
    ...
    def search_for_closest_class_for_given_subject(self, subject: str, class_type: str):
        ...
        return self.check_if_closest_date_exists(dates_after)


    def user_chose_option_2(self):
        class_type_and_subject = self.get_class_type_and_subject_name()
        (class_type, subject, class_type_str) = class_type_and_subject

        date = self.search_for_closest_class_for_given_subject(subject, class_type)
